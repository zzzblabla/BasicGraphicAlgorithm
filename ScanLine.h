...
public:
	...
	void ConstructEdgeTable(const std::vector<int> &vx, const std::vector<int> &vy) {
		...
		for (int i = 0; i < vx.size(); i++) {
			...
			if (vy[i] != vy[(i + 1) % vy.size()]) {
				edge_table_.push_back(
                    Edge(Point(vx[i], vy[i]),
                    Point(vx[(i + 1) % vy.size()], vy[(i + 1) % vy.size()])));
			}
		}
		std::sort(edge_table_.begin(), edge_table_.end(),
            [](Edge e1, Edge e2) {return e1.y_min_ < e2.y_min_; });
	}
	...
	void UpdateAEL(int y) {
		...
		std::sort(active_edge_list_.begin(), active_edge_list_.end(), 
            [](Edge e1, Edge e2) {
                return e1.x_of_y_min_ < e2.x_of_y_min_ || 
                    (e1.x_of_y_min_ == e2.x_of_y_min_&&e1.slash_slope_ < e2.slash_slope_);
            });
		for (int i = 0; i < active_edge_list_.size(); i += 2) {
			range_within_polygon_area_.push_back(std::pair<int, std::pair<int, int>>(
                y, std::pair<int, int>(
                active_edge_list_[i].x_of_y_min_, active_edge_list_[i + 1].x_of_y_min_)));
		}
	}
	...
	void ScanPolygonArea() {
		...
		for (int i = 0; i < range_within_polygon_area_.size(); i++) {
			num += 
                range_within_polygon_area_[i].second.second - range_within_polygon_area_[i].second.first;
		}
